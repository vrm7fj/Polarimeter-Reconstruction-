...
void polarimeter_recon() {
  ...
  TString filename2 = "/volatile/halla/sbs/vidura/GEP_REPLAYS/GEP1/LH2/LH2_START_OVER_IT4/rootfiles/gep5_fullreplay_*";
  //TString filename2 = "/volatile/halla/sbs/vidura/GEP_REPLAYS/GEP1/LH2/LH2_START_OVER/rootfiles/gep5_fullreplay_*";
  ...
  TCut globalcut_thetafpp = "abs(heep.dt_ADC-14.75)<12&&(sbs.gemFT.track.nhits[0]>4||sbs.gemFT.track.ngoodhits[0]>2)&&(sbs.gemFPP.track.nhits[0]>4||sbs.gemFPP.track.ngoodhits[0]>2)&&sqrt(pow((heep.dxECAL[0]+0.002232+0.025*earm.ecal.x[0])/0.01874,2)+pow((heep.dyECAL[0]+0.001319-(0.02781+0.009648*earm.ecal.x[0]+0.008797*pow(earm.ecal.x[0],2)+0.0129*pow(earm.ecal.x[0],3)))/0.02143,2))<=3.5&&sqrt(pow((sbs.gemFPP.track.y[0]+sbs.gemFPP.track.yp[0]*6.7-sbs.hcal.y[0]-0.01672)/0.06043,2)+pow((sbs.gemFPP.track.x[0]+sbs.gemFPP.track.xp[0]*6.7-sbs.hcal.x[0]-0.1842)/0.06863,2))<=3.5&&earm.ecal.nblk[0]>2&&sbs.hcal.nblk[0]>1&&abs(sbs.tr.vz[0]+0.1)<0.15&&abs(heep.dpp+0.02499)<0.02098*3&&(heep.ecalo[0]/heep.eprime_eth[0])>0.7";
  ...
  c->SetBranchStatus("heep.ecalo", 1);
  c->SetBranchStatus("heep.eprime_eth", 1);
  c->SetBranchStatus("heep.dyECAL", 1);
  c->SetBranchStatus("heep.dyECAL", 1);
